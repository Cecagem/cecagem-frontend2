...
const colaboradoresEjemplo: ColaboradorOption[] = [
  { id: "colab-1", label: "Juan Pérez", especialidad: "Metodología de Investigación", tipo: TipoColaborador.INTERNO },
  { id: "colab-2", label: "Ana García", especialidad: "Desarrollo de Software", tipo: TipoColaborador.INTERNO },
  { id: "colab-3", label: "Carlos López", especialidad: "Consultoría Financiera", tipo: TipoColaborador.EXTERNO },
  { id: "colab-4", label: "María Rodríguez", especialidad: "Análisis Estadístico", tipo: TipoColaborador.EXTERNO },
];
...
export function ProyectoForm({ proyecto, onSubmit, onCancel }: ProyectoFormProps) {
  ...
  const {
    ...
    defaultValues: {
      ...
      pagoColaborador: proyecto?.pagoColaborador?.toString() || "",
  ...
  // Función para determinar si el colaborador seleccionado es externo
  const isColaboradorExterno = () => {
    if (!watchColaboradorId) return false;
    const colaborador = colaboradoresEjemplo.find(c => c.id === watchColaboradorId);
    return colaborador?.tipo === TipoColaborador.EXTERNO;
  };
  ...
  const handlePasoSiguiente = async () => {
    ...
    switch (pasoActual) {
      case 1:
        ...
        // Validación especial para colaborador externo
        if (isColaboradorExterno()) {
          const pagoColaboradorValue = watch("pagoColaborador");
          if (!pagoColaboradorValue || parseFloat(pagoColaboradorValue) <= 0) {
            // Marcar error manualmente
            trigger("pagoColaborador");
            return;
          }
        }
        break;
  ...
  const onFormSubmit = (data: ProyectoFormData) => {
    ...
    const proyectoData: Proyecto = {
      ...
      pagoColaborador: data.pagoColaborador ? parseFloat(data.pagoColaborador) : undefined,
      ...
      <div className="space-y-4 sm:space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
          ...
          <div className="space-y-3">
            ...
            {/* Campo de pago del colaborador - Solo visible para colaboradores externos */}
            {isColaboradorExterno() && (
              <div>
                <Label htmlFor="pagoColaborador" className="text-sm font-medium">
                  Pago al Colaborador Externo * ({watchMoneda === Moneda.DOLARES ? 'USD' : 'PEN'})
                </Label>
                <Controller
                  ...
                  rules={{
                    validate: (value) => {
                      if (!isColaboradorExterno()) return true;
                      if (!value || value.trim() === "") {
                        return "El pago al colaborador externo es requerido";
                      }
                  ...
                  render={({ field }) => (
                    <div className="relative mt-1.5">
                      ...
                      <Input
                        ...
                        step="0.01"
                        placeholder="0.00"
                        className={cn("pl-10", errors.pagoColaborador ? "border-red-500" : "")}
                      />
                    </div>
                  )}
                />
                {errors.pagoColaborador && (
                  <p className="text-sm text-red-500 mt-1">{errors.pagoColaborador.message}</p>
                )}
                <p className="text-xs text-muted-foreground mt-1">
                  Monto a pagar al colaborador externo por este proyecto
                </p>
              </div>
            )}
